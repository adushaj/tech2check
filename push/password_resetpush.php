    ...
    if (isset($_POST['btn-update'])) {
        ...
        while ($row = mysql_fetch_array($result)) {
            if (!password_verify($old_pass, $row['password'])) {
                $_SESSION['re_error'] = "Incorrect password!";
                
                header("location: ../user_account.php");
                exit;
            } elseif ($old_pass == $row['password'] || password_verify($old_pass, $row['password'])) {
                if ($new_pass != $verify) {
                    $_SESSION['re_error'] = "Password not verified!";
                ...
                if (mysql_query($sql)) {
                    $_SESSION['re_success'] = "Password reset successfully!";
        ...
        $_SESSION['re_error'] = "Error occurred!";
